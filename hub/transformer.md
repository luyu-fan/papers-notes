...
| 2021 | arxiv | [RealFormer: Transformer Likes Residual Attention](../notes/transformer_realformer.md) | 规范化层的位置和类型，更好更快地训练transformer结构。 | 类似residual机制，对相似度值进行计算后，和之前的相似度值相加再计算权重分布。 | 既可以得到pre-norm的稳定性也可以得到post-norm的结果。        | 预训练的使用使用的比较大的学习率，warm-up, AdamW，微调的时候使用的学习率很小。 | 非常简单直白，且看起来有很好的提升效果。                     |
